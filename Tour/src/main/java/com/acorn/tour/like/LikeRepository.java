...
public class LikeRepository {
	...
	public ArrayList<LikeDTO> myLike(String userId) {
		...
		String sql = "select li.userid, li.contentid, ct.contenttypeid, ct.title, "
				   + "si.sidocode, si.nickname, sg.sigungucode, sg.name, ct.firstimage, ct.cat2 "
				   + "from liketbl li "
		...
		try {
			...
			while(rs.next()) {
				...
				LikeDTO item = new LikeDTO(userId, contentid, contenttypeid, title, sidocode, nickname, sigungucode, sigunguname, firstimage, cat2);
				list.add(item);
			}
